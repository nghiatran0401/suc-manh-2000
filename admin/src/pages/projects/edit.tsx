...
export const ProjectEdit: React.FC<IResourceComponentsProps> = () => {
  ...
  const isProject =
    collectionName.includes("du-an") ||
    collectionName.includes("phong-tin-hoc");
  ...
  const { formProps, saveButtonProps, queryResult } = useForm({
    ...
    successNotification(data: any, values: any) {
      const messageHtml = `<a target="_blank" href="${
        CLIENT_URL + "/" + data?.data?.category + "/" + data?.data?.slug
      }">${data?.data?.name}</a>`;
      return {
  ...
  useEffect(() => {
    if (projectData) {
      formProps.form?.setFieldValue(
        "totalFund",
        projectData.totalFund ? projectData.totalFund / 1000000 : 0
      );
      formProps.form?.setFieldValue(
        "publish_date",
        projectData.publish_date ? projectData.publish_date.split("T")[0] : ""
      );
      formProps.form?.setFieldValue(
        "start_date",
    ...
    <Edit saveButtonProps={saveButtonProps}>
      ...
      >
        ...
        <Form.Item
          label={
            <span style={{ fontSize: "16px", fontWeight: "bold" }}>
              {translate("post.fields.publish_date")}
            </span>
          }
          name={"publish_date"}
          rules={[{ required: true }]}
        ...
        {isProject && (
          <Form.Item
            label={
              <span style={{ fontSize: "16px", fontWeight: "bold" }}>
                {translate("post.fields.totalFund")}
              </span>
            }
            name={"totalFund"}
            rules={[{ required: true }]}
